#[test_only]
module gitto_addr::test_end_to_end {
    use std::option;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    #[test(aptos_framework = @0x1, sender = @gitto_addr, user1 = @0x200, user2 = @0x201)]
    fun test_happy_path(
        ...
        identity::init_module_for_test(sender);

        identity::create_collection(
            sender,
        ...
        let registry = identity::get_registry();
        let collection_1 = *vector::borrow(&registry, vector::length(&registry) - 1);
        assert!(collection::count(collection_1) == option::some(3), 1);

        let mint_fee = identity::get_mint_fee(collection_1, string::utf8(ALLOWLIST_MINT_STAGE_CATEGORY), 1);
        aptos_coin::mint(aptos_framework, user1_addr, mint_fee);

        identity::mint_nft(user1, collection_1, 1);

        let active_or_next_stage = identity::get_active_or_next_mint_stage(collection_1);
        assert!(active_or_next_stage == option::some(string::utf8(ALLOWLIST_MINT_STAGE_CATEGORY)), 3);
        let (start_time, end_time) = identity::get_mint_stage_start_and_end_time(
            collection_1,
    ...
    #[test(aptos_framework = @0x1, sender = @gitto_addr, user1 = @0x200)]
    #[expected_failure(abort_code = 12, location = identity)]
    fun test_mint_disabled(
        ...
        assert!(identity::is_mint_enabled(collection_1), 1);
        ...
        identity::update_mint_enabled(sender, collection_1, false);
        assert!(!identity::is_mint_enabled(collection_1), 2);
