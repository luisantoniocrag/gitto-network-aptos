module gitto_addr::identity {
    ...
    public entry fun update_creator(sender: &signer, new_creator: address) acquires Config {
        ...
        let config = borrow_global_mut<Config>(@gitto_addr);
    ...
    public entry fun create_collection(
        ...
        let sender_addr = signer::address_of(sender);
        let config = borrow_global<Config>(@gitto_addr);
        assert!(
        ...
        let collection_owner_obj_constructor_ref = &object::create_object(@gitto_addr);
        ...
        let registry = borrow_global_mut<Registry>(@gitto_addr);
    ...
    public fun get_registry(): vector<Object<Collection>> acquires Registry {
        let registry = borrow_global<Registry>(@gitto_addr);
    ...
    fun is_admin(config: &Config, sender: address): bool {
        if (sender == config.admin_addr) {
            ...
            if (object::is_object(@gitto_addr)) {
                let obj = object::address_to_object<ObjectCore>(@gitto_addr);
    ...
    fun mint_nft_internal(
        ...
        let nft_metadata_uri = construct_nft_metadata_uri(&collection_uri);

        // use the collection name + next nft id as the token name
        let token_name  = string_utils::format2(&b"{} #{}", collection::name(collection_obj), next_nft_id);

        let nft_obj_constructor_ref = &token::create_named_token(
            collection_owner_obj_signer,
    ...
    fun construct_nft_metadata_uri(
        ...
        let nft_metadata_filename = string_utils::format1(&b"{}.json", 1);
